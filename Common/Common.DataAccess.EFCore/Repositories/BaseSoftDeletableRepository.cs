...
{
    public abstract class BaseSoftDeletableRepository<TType, TContext>
        where TType : BaseEntity, new()
        where TContext : DataContext
    {
        private readonly TContext _dbContext;

        protected BaseSoftDeletableRepository(TContext context)
        {
            _dbContext = context;
        }
        ...
        protected IQueryable<TType> GetEntities(ContextSession session, bool includeDeleted = false)
        {
            ...
            if (!includeDeleted)
            {
                query = query.Where(obj => obj.DeletedAt == null);
            }
