...
namespace Common.WebApiCore
{
    public class JwtManager
    {
        ...
        public JwtManager(IOptions<JwtOption> jwtOptions)
        {
        ...
        public Token GenerateTokenExternal<TUser>(TUser user) where TUser : User
        {
            var token = new Token
            {
                Expires_in = jwtOptions.AccessValidFor.TotalMilliseconds,
                Access_token = CreateToken(user, jwtOptions.AccessExpireExternal, jwtOptions.AccessExternalSigningCredentials),
                Refresh_token = CreateToken(user, jwtOptions.RefreshExpiration, jwtOptions.RefreshSigningCredentials)
            };
